...
typedef struct dummy_data_bl_queue {
    int data;
    bl_queue_node_t node;
} dummy_data_bl_queue_t;

typedef struct test_data_bl_queue {
    int thread_id;
    int count_enque_ops;
    int count_deque_ops;
    int number_ops;
    bl_queue_head_t *q;
    dummy_data_bl_queue_t* dummy_data;
    long start_enq;
    long end_enq;
    long start_deq;
    long end_deq;
} test_data_bl_queue_t;

void* test_func_bl_queue(void* thread_data) {
    LOG_PROLOG();

    test_data_bl_queue_t* data = (test_data_bl_queue_t*)thread_data;
    ...
    int i = 0;
    for (i = 0; i < data->count_enque_ops; ++i) {
        bl_enqueue(data->q, &(data->dummy_data[i].node));
    }
    ...
    bl_queue_node_t* x = NULL;
    dummy_data_bl_queue_t* val = NULL;
    int perThreadSanityFails = 0;
    
    data->start_deq = usecs();
    for (i = 0; i < data->count_deque_ops; ++i) {
        x = bl_dequeue(data->q);
        val = (dummy_data_bl_queue_t*) list_entry(x, dummy_data_bl_queue_t, node);
...
void test_bl_queue(int count_threads, int enq_ops, int deq_ops) {
    
    LOG_PROLOG();

    printf("\n*********** BLOBKING ************\n");
    dummy_data_bl_queue_t *dummy_data = (dummy_data_bl_queue_t*) malloc(sizeof(dummy_data_bl_queue_t) * (count_threads*enq_ops + 1));

    int i = 0;
    for (i = 0; i < (count_threads*enq_ops + 1); ++i) {
        dummy_data[i].data = i;
        init_bl_queue_node(&(dummy_data[i].node));
        dummy_data[i].node.sanityData = i;
    }

    bl_queue_head_t *q = create_bl_queue();
    pthread_t *threads = (pthread_t*)malloc(sizeof(pthread_t) * count_threads);;
    pthread_barrier_init(&barr_enq, NULL, count_threads);
    pthread_barrier_init(&barr_deq, NULL, count_threads);

    test_data_bl_queue_t *thread_data = (test_data_bl_queue_t*)malloc(sizeof(test_data_bl_queue_t) * count_threads);
    ...
    for (i = 0; i < count_threads; ++i) {
        ...
        pthread_create(threads + i, NULL, test_func_bl_queue, thread_data+i);
    }
    ...
    bl_queue_node_t *x = q->head;
    int total=0;
    int sanityFails = 0;
    while(x!=NULL){
        dummy_data_bl_queue_t* val = (dummy_data_bl_queue_t*)list_entry(x, dummy_data_bl_queue_t, node);
        if (val->data > (count_threads*enq_ops)) {
            LOG_WARN("Invalid value in node - %d", val->data);
        }
        else {
...
int main(int argc, char** argv) {
    ...
    test_bl_queue(count_threads, enq_ops, deq_ops);
